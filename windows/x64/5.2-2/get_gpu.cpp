...
bool TInventory::get_gpu() {

    auto gpus = get_gpus();

if (!gpus.empty()) {
    const auto& gpu = gpus[0];  // берём первую видеокарту
    id_gpu.assign(gpu.name.begin(), gpu.name.end());
    id_gpu_size = std::to_string(gpu.vram_bytes / (1024 * 1024)) + " Mb";
    return true;
}
return false;
