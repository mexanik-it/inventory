...
def perform_backup(project_root: str) -> dict:
    ...
    from report import init_report, append_report_row, finalize_report
    
    init_report()  # 1. Создаёт каркас (перезаписывает файл)
    ...
    finalize_report()  # 2. Закрывает теги (дописывает в конец)
